...
**致歉声明：** 此前版本在 `WorldviewItem` 的设计和 `Experience` 与 `Chapter` 的关联上存在严重缺陷。本最终版已根据您的全部需求，对“关系与上下文域”进行了关键补充，以确保设计完全符合您的业务逻辑。
...
## 第三部分：创作资产域 (Creative Asset Domain)
...
## 第五部分：关系与上下文域 (Relationship & Context Domain)
