...
export function TiptapEditor({
  ...
  const toggleAI = () => setInternalIsAIOpen(!isAIOpen);
        ...
        >
          ...
          >
            {settings.customTitleStyle ? (
              ...
              <div className="flex items-end gap-2 text-xl font-medium text-foreground/80">
      ...
      <div className="fixed bottom-6 right-4 sm:bottom-8 sm:right-8 z-50">
        {isDesktop ? (
           <Button
             ...
             size="icon"
             className={cn(
               "h-12 w-12 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 z-50",
               isAIOpen 
      ...
      <>
        <ResizablePanelGroup direction="horizontal" className="h-full group/panels">
          ...
          <ResizablePanel 
            ...
            minSize={20} 
            maxSize={45} 
            onCollapse={() => setInternalIsAIOpen(false)}
            onExpand={() => setInternalIsAIOpen(true)}
            className={cn(
              "transition-[flex] duration-500 ease-in-out shadow-2xl z-40 theme-" + settings.theme,
              !isAIOpen && "border-none"
            )}
          >
            <div className="h-full w-full overflow-hidden editor-paper">
                <AIChatInterface
                  workId={workId ? parseInt(workId, 10) : undefined}
                  selectedText={selectedText}
