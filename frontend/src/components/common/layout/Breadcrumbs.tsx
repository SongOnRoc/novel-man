...
export function Breadcrumbs({ className = "" }: { className?: string }) {
  ...
  const crumbs = segments.map((seg, idx) => {
    ...
    // Check if this segment is a Chapter ID (follows "chapters")
    // If so, it shouldn't be a link because there is no /chapters/[id] page
    const isChapterId = idx > 0 && segments[idx - 1] === "chapters";
    const isNavigable = !isChapterId;
